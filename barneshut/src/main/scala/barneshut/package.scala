...
package object barneshut {
  ...
  extends Quad {
    val mass: Float = bodies.foldLeft(0f)(_+_.mass): Float
    val massX:Float = bodies.foldLeft(0f)((acc:Float, b:Body) => acc + b.mass * b.x)/mass : Float
    val massY:Float = bodies.foldLeft(0f)((acc:Float, b:Body) => acc + b.mass * b.y)/mass : Float
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val sector = norY(tY(b.y)).toInt * sectorPrecision + normX(tX(b.x)).toInt
      matrix(sector.toInt) += b
      this
    }

    // SR73
     def norm(number: Float, min: Float, max: Float): Float = {
      (number - min)/(max-min)
    }

    def normX(x: Float): Float = {
      norm(x, boundaries.minX, boundaries.maxX) * sectorPrecision
    }

    def norY(y: Float): Float = {
      norm(y, boundaries.minY, boundaries.maxY) * sectorPrecision
    }

    def tX(x: Float): Float = {
      if(x < boundaries.minX) boundaries.minX
      else if(x >= boundaries.maxX) boundaries.maxX-1
      else x
    }

    def tY(y: Float): Float = {
      if(y < boundaries.minY) boundaries.minY
      else if(y >= boundaries.maxY) boundaries.maxY-1
      else y
    }
    // SR73
