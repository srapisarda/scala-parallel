...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    input.zipWithIndex.foreach{
      case (x, 0) => output(0) = 0
      case (x, i) => output(i) = Math.max(x / i, output(i-1))
    }
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    def downsweepSequential0(i: Int, end: Int, max:Float): Unit = {
      if(i < end){
        output(i) = List(input(i) / i, max).max
        downsweepSequential0(i+1, end, output(i))
      }
    }

    downsweepSequential0(from, until, startingAngle)
  }
